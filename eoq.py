...
def calculate_dynamic_eoq(
    ...
    adjusted_order_cost = base_order_cost + (labor_cost_per_hour * time_per_order_hours)
    adjusted_holding_cost = cogs * holding_cost_rate
...
base_order_cost = st.sidebar.number_input("Base Ordering Cost per Order (USD)", value=200.0, step=10.0)
labor_cost_per_hour = st.sidebar.number_input("Labor Cost per Hour (USD)", value=25.0, step=1.0)
time_per_order_hours = st.sidebar.number_input("Time Required per Order (hours)", value=2.0, step=0.1)
